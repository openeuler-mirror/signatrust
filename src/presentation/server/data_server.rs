...
impl DataServer {
    ...
    async fn load(&mut self) -> Result<()> {
        ...
        if !file_exists(&ca_root) || !file_exists(&tls_cert) || !file_exists(&tls_key) {
            return Err(Error::FileFoundError(format!(
                "ca root: {} or tls cert: {} or key: {} file not found",
                ca_root, tls_key, tls_cert
            )));
        }
        self.ca_cert = Some(Certificate::from_pem(fs::read(ca_root).await?));
        self.server_identity = Some(Identity::from_pem(
            fs::read(tls_cert).await?,
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        let sign_backend =
            SignBackendFactory::new_engine(self.server_config.clone(), get_db_connection()?)
                .await?;
        let data_repository = repository::DataKeyRepository::new(get_db_connection()?);
