...
use rpm::{Header, IndexSignatureTag, Package, Digests, IndexTag};
...
impl RpmFileHandler {
    ...
    fn generate_v3_signature(&self, sign_options: &HashMap<String, String>, package: &Package) -> bool {
        if let Some(v3_format) = sign_options.get(options::RPM_V3_SIGNATURE) {
            if v3_format == "true" {
                return true;
            }
        }
        if package.metadata.header.entry_is_present(IndexTag::RPMTAG_PAYLOADDIGEST) || package.metadata.header.entry_is_present(IndexTag::RPMTAG_PAYLOADDIGESTALT) {
            return false;
        }
        true
    }
