<template>
  ...
  >
    ...
    <el-table-column fixed="right" label="Operations" width="400">
      <template #default="scope">
        ...
        <el-button link type="primary" size="small" @click="enableData(scope.row.id, 'enable')"
          >Enable</el-button
        ></span> <span v-if="scope.row.key_state === 'enabled'">|
        <el-button link type="primary" size="small" @click="disableData(scope.row.id, 'disable')"
          >Disable</el-button
        ></span><span v-if="scope.row.key_state === 'disabled'"
          >|
          <el-button
            link
            type="primary"
        ...
        ><span v-else
          >|
          <el-button link type="" size="small" disabled
            >Request Delete</el-button
          ></span
        >
      </template>
  ...
  <div class="demo-pagination-block">
    <el-pagination
      ...
      v-model:page-size="useData.paginationPri.pageSize"
      :page-sizes="[10, 20, 50]"
      background
      layout="sizes,prev, pager, next,slot, jumper"
      :total="useData.paginationPri.totalCount"
      @current-change="handleCurrentChange"
      @size-change="sizeChange"
      ><span
        >{{ useData.paginationPri.currentPage }}/{{
          Math.ceil(useData.paginationPri.totalCount / useData.paginationPri.pageSize)
        }}</span
      ></el-pagination>
  </div>
...
const handleCurrentChange = (val: any) => {
  ...
  if (val?.isTrusted) {
    ...
    useData.paginationPri.currentPage = val;
    useData.getPriTableData();
  }
};
// 改变每页显示条数
const sizeChange = (val: any) => {
  useData.paginationPri.pageSize = val;
